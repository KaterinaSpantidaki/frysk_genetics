#!/bin/sh
#SBATCH --job-name=chr4_PGS_prep_geno_input_filter.PLINK
...
#---------------------------------------
plink \
--bfile $GRMDir/4.All.filtered \
--keep $WorkingDir/output/relatedness.0.125_individuals.grm.id \
--make-bed \
--out $WorkingDir/output_PLINK/All_ind.exc_chr4
...
plink \
--bfile $WorkingDir/output_PLINK/All_ind.exc_chr4 \
--maf 0.01 \
--hwe 0.000001 \
--geno 0.01 \
--mind 0.01 \
--make-bed \
--out $WorkingDir/output_PLINK/All_ind.exc_PLINK_chr4

# Check and remove ambiguous SNPs
awk '($5 == "A" && $6 == "T") || ($5 == "T" && $6 == "A") || ($5 == "C" && $6 == "G") || ($5 == "G" && $6 == "C") {print $2}' $WorkingDir/output_PLINK/All_ind.exc_PLINK_chr4.bim > $WorkingDir/output_PLINK/ambiguous_snps_chr4.txt

plink \
--bfile $WorkingDir/output_PLINK/All_ind.exc_PLINK_chr4 \
--exclude $WorkingDir/output_PLINK/ambiguous_snps_chr4.txt \
--make-bed \
--out $WorkingDir/output_PLINK/All.chr4.filtered
