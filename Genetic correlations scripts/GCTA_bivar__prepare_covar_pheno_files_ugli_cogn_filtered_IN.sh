...
pheno <- pheno %>% select("project_pseudo_id", "gender", "age_speech", "age_multilingualism", "foreign_speech_imitation", "n_fluent_languages")
pheno <- pheno %>%
  filter(!(is.na(foreign_speech_imitation) & is.na(n_fluent_languages)))
...
write.table(covar_ugli_cyto_binary, "/groups/umcg-lifelines/tmp02/projects/ov21_0398/GCTA/frysk/gen_cor/input_files/covar_binary_ugli1and2_cyto_bivar_cogn_filtered_IN.txt", row.names = FALSE, col.names = FALSE, quote=FALSE, sep="\t")
...
write.table(covar_ugli_cyto_quant, "/groups/umcg-lifelines/tmp02/projects/ov21_0398/GCTA/frysk/gen_cor/input_files/covar_quant_ugli1and2_cyto_bivar_cogn_filtered_IN.txt", row.names = FALSE, col.names = FALSE, quote=FALSE, sep="\t")
...
data2_clean <- data2_unique %>%
   select("FID_GRM", "IID_GRM", "foreign_speech_imitation", "n_fluent_languages")
write.table(data2_clean, 
              file = paste0(output_herit, "pheno_ugli_cyto_bivar_cogn_filtered_IN.txt"),  
              row.names = FALSE, 
              col.names = FALSE,
			  quote=FALSE,			  
              sep = "\t")
