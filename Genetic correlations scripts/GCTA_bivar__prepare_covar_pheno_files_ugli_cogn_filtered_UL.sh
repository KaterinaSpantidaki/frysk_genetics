...
pheno <- pheno %>% select("project_pseudo_id", "gender", "age_speech", "foreign_accents_understanding", "foreign_language_learning")
pheno <- pheno %>%
  filter(!(is.na(foreign_accents_understanding) & is.na(foreign_language_learning)))
...
write.table(covar_ugli_cyto_binary, "/groups/umcg-lifelines/tmp02/projects/ov21_0398/GCTA/frysk/gen_cor/input_files/covar_binary_ugli1and2_cyto_bivar_cogn_filtered_UL.txt", row.names = FALSE, col.names = FALSE, quote=FALSE, sep="\t")
...
write.table(covar_ugli_cyto_quant, "/groups/umcg-lifelines/tmp02/projects/ov21_0398/GCTA/frysk/gen_cor/input_files/covar_quant_ugli1and2_cyto_bivar_cogn_filtered_UL.txt", row.names = FALSE, col.names = FALSE, quote=FALSE, sep="\t")
...
data2_clean <- data2_unique %>%
   select("FID_GRM", "IID_GRM", "foreign_accents_understanding", "foreign_language_learning")
write.table(data2_clean, 
              file = paste0(output_herit, "pheno_ugli_cyto_bivar_cogn_filtered_UL.txt"),  
              row.names = FALSE, 
              col.names = FALSE,
			  quote=FALSE,			  
              sep = "\t")
